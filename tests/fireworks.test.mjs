...
let energy=0;const engine={context:{state:'running',sampleRate:48000},analyser:{getByteFrequencyData:bins=>bins.fill(energy),getByteTimeDomainData:wave=>wave.fill(128+Math.round(energy/3))}};
...
settings.musicPaused=true;advance(12);assert.equal(advance(2),90*120,'paused music never falls back to demo or stale FFT launches');settings.musicPaused=false;
...
stop();assert.equal(cancelled,true);assert.equal(listeners.size,0);console.log('6 animation behavior checks passed');
