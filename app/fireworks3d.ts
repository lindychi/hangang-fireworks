...
type Shell = { p:THREE.Vector3; target:THREE.Vector3; color:THREE.Color; kind:number; power:number; seed?:number };
...
export function startFireworks3D(canvas:HTMLCanvasElement,settings:Settings,getEngine:()=>SoundEngine|null){
  ...
  renderer.setPixelRatio(Math.min(devicePixelRatio,1.7));renderer.setSize(innerWidth,innerHeight);renderer.setClearColor('#101e34');
  const scene=new THREE.Scene();scene.fog=new THREE.FogExp2('#1a2b43',.0009);
  ...
  scene.add(new THREE.HemisphereLight('#7891b3','#293b30',1.35));
  const moonLight=new THREE.DirectionalLight('#b5c9ed',1.1);moonLight.position.set(40,90,-160);scene.add(moonLight);
  ...
  for(let i=0;i<85;i++){
    const x=(i-42)*10,bh=rand(5,24),z=rand(-365,-335);box(x,bh/2,z,rand(5,9),bh,8,'#30425b');
  ...
  const windowsGeo=new THREE.BufferGeometry();windowsGeo.setAttribute('position',new THREE.Float32BufferAttribute(windowPositions,3));scene.add(new THREE.Points(windowsGeo,new THREE.PointsMaterial({color:'#e2bf7a',size:.65,transparent:true,opacity:.9})));
  box(-170,7,-185,245,1.2,10,'#526b85');
  for(let x=-290;x<-45;x+=24){box(x,2.7,-185,2,7,4,'#43566e');const bulb=new THREE.Mesh(new THREE.SphereGeometry(.5,6,6),new THREE.MeshBasicMaterial({color:'#ffcd83'}));bulb.position.set(x,9,-179);scene.add(bulb);}
  ...
  const waterMaterial=new THREE.ShaderMaterial({transparent:false,uniforms:{time:{value:0},flashes:{value:flashes},flashColors:{value:flashColors}},vertexShader:`varying vec3 world; void main(){vec4 p=modelMatrix*vec4(position,1.);world=p.xyz;gl_Position=projectionMatrix*viewMatrix*p;}`,fragmentShader:`
    ...
    void main(){vec2 p=world.xz;float wave=sin(p.x*.85+p.y*.75+time*1.1)*.5+sin(p.y*2.8-time*1.7)*.3+sin(p.x*3.+p.y*.3+time)*.2;
      vec3 col=vec3(.035,.07,.115)+vec3(.025,.044,.065)*max(0.,wave);vec3 view=normalize(cameraPosition-world);vec3 normal=normalize(vec3(sin(p.x*.85+time)*.07,1.,cos(p.y*.7-time)*.13));
      for(int i=0;i<8;i++){vec3 source=flashes[i].xyz;vec3 light=normalize(source-world);vec3 halfway=normalize(light+view);float spec=pow(max(dot(normal,halfway),0.),38.);float band=exp(-pow((p.x-source.x)/(6.+abs(p.y-source.z)*.15),2.));float reach=smoothstep(source.z-15.,source.z+25.,p.y);col+=flashColors[i]*flashes[i].w*reach*(spec*1.8+band*(.06+max(0.,wave)*.14));}
      float haze=smoothstep(100.,450.,length(p-cameraPosition.xz));gl_FragColor=vec4(mix(col,vec3(.06,.09,.14),haze*.55),1.);
    }`});
  ...
  const particleMaterial=new THREE.ShaderMaterial({transparent:true,depthWrite:false,blending:THREE.AdditiveBlending,vertexColors:true,uniforms:{pixelRatio:{value:renderer.getPixelRatio()}},vertexShader:`attribute float size; uniform float pixelRatio; varying vec3 tint; void main(){tint=color;vec4 mv=modelViewMatrix*vec4(position,1.);gl_PointSize=clamp(size*480.*pixelRatio/-mv.z,1.,55.);gl_Position=projectionMatrix*mv;}`,fragmentShader:`varying vec3 tint;void main(){float d=length(gl_PointCoord-.5)*2.;if(d>1.)discard;float a=exp(-d*d*4.);gl_FragColor=vec4(tint*(1.+exp(-d*d*35.)*1.8),a);}`});
  ...
  let embers:Ember[]=[],shells:Shell[]=[],frame=0,last=0,time=0,lastLaunch=0,flashIndex=0,frameCounter=0;
  const director=createMusicDirector(),readMusic=createMusicReader(),effects=createFireworkSound(settings,getEngine);
  function launch(power=1,x=rand(-80,80),height=rand(55,110),z=rand(-170,-80),kind=Math.floor(rand(0,4)),hue=kind===1?42:[35,44,335,195,275][Math.floor(rand(0,5))],seed?:number){
    if(shells.length>12)return;const color=new THREE.Color().setHSL(hue/360,.85,.72);shells.push({p:new THREE.Vector3(x*.6,1,z),target:new THREE.Vector3(x,height,z),color,kind,power,seed});effects.play('launch',power,x/150);
  }
  function burst(shell:Shell){
    const rand=shell.seed===undefined?(a:number,b:number)=>a+Math.random()*(b-a):seededRandom(shell.seed);
    const count=Math.floor(260*shell.power),willow=shell.kind===1;
    for(let i=0;i<count;i++){
      const theta=shell.kind===3?(i%5)/5*Math.PI*2+rand(-.07,.07):rand(0,Math.PI*2),cos=rand(-1,1),sin=Math.sqrt(1-cos*cos);const speed=(shell.kind===2?27:rand(13,31))*shell.power;
      const v=shell.kind===3?new THREE.Vector3(Math.cos(theta)*speed,Math.sin(theta)*speed,rand(-3,3)):shell.kind===2?new THREE.Vector3(Math.cos(theta)*speed,Math.sin(theta)*speed,rand(-1,1)):new THREE.Vector3(Math.cos(theta)*sin*speed,cos*speed,Math.sin(theta)*sin*speed);
      const life=willow?rand(3.2,5):rand(1.6,3.2);embers.push({p:shell.p.clone(),v,color:shell.color,life,max:life,size:rand(.48,1.05),trail:[],willow});
    }
    ...
    light.position.copy(shell.p);light.color.copy(shell.color);light.intensity=500*shell.power;effects.play('burst',shell.power,shell.p.x/150+Math.sin(yaw),shell.kind===3);
  }
  function fan(seed?:number){const rand=seed===undefined?(a:number,b:number)=>a+Math.random()*(b-a):seededRandom(seed);for(let x=-64;x<=64;x+=32)for(let i=0;i<35;i++){const a=-Math.PI/2+(i/34-.5)*1.6;const life=rand(.8,1.5);embers.push({p:new THREE.Vector3(x,1,-125),v:new THREE.Vector3(Math.cos(a)*25,-Math.sin(a)*25,rand(-3,3)),color:new THREE.Color('#ffc478'),life,max:life,size:.35,trail:[],willow:false});}}
  ...
  function render(now:number){
    ...
    effects.sync();if(settings.paused)director.reset();
    if(!settings.paused){time+=dt;frameCounter++;
      ...
      if(settings.connected){
        if(e&&!settings.musicPaused&&e.context.state==='running')for(const cue of director.update(readMusic(e.analyser,e.context.sampleRate),now,settings.intensity)){
          launch(cue.power,(cue.x-.5)*230,35+cue.height*95,-170+cue.depth*90,cue.kind,cue.hue,cue.seed);if(cue.fan)fan(cue.seed);
        }else director.reset();
      }else {director.reset();if(time-lastLaunch>1.35/settings.intensity){launch(rand(.8,1.4));lastLaunch=time;if(Math.random()>.6){launch(1,-55,70,-130,1);launch(1,55,70,-130,1);fan();}}}
    ...
    let count=0;
    for(const p of embers){const alpha=Math.min(1,p.life/p.max*1.4)*(p.willow&&p.life<1?.65+.35*Math.sin(p.life*50):1);if(count>=capacity-12)break;write(p.p,p.color,alpha,p.size,count++);p.trail.forEach((pos,i)=>write(pos,p.color,alpha*(1-i/(p.trail.length+1))*.6,p.size*.65,count++));}
  ...
  if(!settings.connected){launch(1.4,25,75,-110,1);launch(1,-55,62,-150,2);}frame=requestAnimationFrame(render);
  return ()=>{cancelAnimationFrame(frame);canvas.removeEventListener('pointerdown',down);canvas.removeEventListener('pointermove',move);canvas.removeEventListener('pointerup',up);canvas.removeEventListener('pointercancel',up);window.removeEventListener('keydown',key);window.removeEventListener('resize',resize);effects.dispose();scene.traverse(o=>{if(o instanceof THREE.Mesh||o instanceof THREE.Points||o instanceof THREE.LineSegments){o.geometry.dispose();const materials=Array.isArray(o.material)?o.material:[o.material];materials.forEach(m=>m.dispose());}});smoke.forEach(p=>p.sprite.material.dispose());smokeTexture.dispose();renderer.dispose();};
}
